...
const ChallengePage = () => {
  ...
  const { challenges, challengeCounts, loading } = useChallenges(selectedStatus);
    ...
    <div className="flex-1 px-6 py-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 mb-4">
        {/* {items.map((item) => (
          ...
          >
            <FileText
              className={`h-4 w-4 ${item.isActive ? "text-[#2B71F0]" : "text-[#98A2B3]"
                }`}
            />
            <span className="text-sm font-medium text-[#344054] text-nowrap">
              {item.label}
            </span>
            <div
              className={`h-6 w-7  rounded-full flex items-center justify-center ${item.isActive
